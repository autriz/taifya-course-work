...
use crate::{ast::{Expression, IdentifierType, Operator, Primitive, Program, Statement}, token::Token};
...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    ...
    fn create_entry_block_alloca(&self, name: &str, name_type: &IdentifierType) -> PointerValue<'ctx> {
        ...
        match &name_type {
            IdentifierType::Int => builder.build_alloca(self.context.i64_type(), name).unwrap(),
            IdentifierType::Float => builder.build_alloca(self.context.f64_type(), name).unwrap(),
            IdentifierType::Bool => builder.build_alloca(self.context.bool_type(), name).unwrap(),
            IdentifierType::String => builder.build_alloca(self.context.ptr_type(AddressSpace::default()), name).unwrap(),
        }
    }
    ...
    fn compile_stmt(&mut self, stmt: &Statement) {
        match stmt {
            Statement::Declaration(decl) => {
                for identifiers in &decl.identifiers {
                    for name in &identifiers.names {
                        ...
                        self.variables.insert(name.to_string(), Variable::new(identifiers.names_type, basic_type, alloca));
                    }
                }
            },
        ...
        match operator {
            ...
            Operator::Input(input) => {
                ...
                let _ = self.builder.build_direct_call(
                    scanf_prototype(&self.context, &self.module), 
                    &args, 
                    "scanf"
                ).unwrap();
            },
            Operator::Output(output) => {
                ...
                for expression in &output.expressions {
                    expressions.push(self.compile_expression(expression).unwrap().into());
                }

                let formatted_string = output.expressions.iter().map(|expression| {
                    let compiled_expr = self.compile_expression(expression).unwrap();

                    println!("{}", compiled_expr.get_type());

                    match &compiled_expr.get_type() {
                        BasicTypeEnum::IntType(_) => "%ld",
                        BasicTypeEnum::FloatType(_) => "%lf",
                        BasicTypeEnum::ArrayType(_) => "%s",
                        BasicTypeEnum::PointerType(_) => "%s",
                        _ => unreachable!("Should not be an another basic type")
                    }
                }).collect::<Vec<&str>>();

                let mut formatted_string = formatted_string.join("");
                ...
                let _ = self.builder.build_direct_call(
                    printf_prototype(&self.context, &self.module), 
                    &args, 
                    "printf"
                ).unwrap();
            },
            ...
            Operator::FixedLoop(loop_) => {
                ...
                let cur_value = self.build_load(variable.pointer, &name, &variable.var_type);
        ...
        match expression {
            Expression::Identifier(ident) => {
                match self.variables.get(&ident.value) {
                    Some(variable) => Ok(self.build_load(variable.pointer, &ident.value, &variable.var_type)),
            ...
            Expression::Primitive(primitive) => match primitive {
                ...
                Primitive::String { value, .. } => {
                    ...
                    // };
                    let string = self.context.const_string(
                        value.as_bytes(), 
                        false
                    );
